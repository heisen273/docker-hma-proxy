...
# Download serverlist
curl -s -k https://vpn.hidemyass.com/vpn-config/l2tp/ > /tmp/serverlist.txt
...
$curl https://s3.amazonaws.com/hma-zendesk/linux/servers-cli.txt 2>/dev/null| grep -i -e "$grep" | grep -i -e "$proto" > /tmp/hma-servers
exec < /tmp/hma-servers

# If serverlist empty 
if [[ "$(cat /tmp/hma-servers)" == "" ]]; then
if [[ "$($curl https://s3.amazonaws.com/hma-zendesk/linux/servers-cli.txt)" == "" ]] ; then
